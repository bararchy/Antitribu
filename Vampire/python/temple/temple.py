...
#TEMPLE1: set temple to non-combat state, changed by wesp
def SetTempleState():
...
#TEMPLE1: Yukie, added by wesp
def YukieFloat(n):
    yukie = Find("Yukie")
    if (yukie and G.Patch_Plus == 1):
...
def tpl3_check_pedestals():
	...
	Find("Jenny").PlayDialogFile("character/dlg/antitribu/Jenny/jenny_16.wav")
...
##          torchtrigger.OnDeactivate()
	...
	Find("Jenny").PlayDialogFile("character/dlg/antitribu/Jenny/jenny_15.wav")
...
#------------------------------------------------------------------
#TEMPLE1: check join to sabbat
def SabbathCheck():
    #if (G.Story_State == 110):
        #if (G.Player_Sabbat_Join == 1):
        #    Find("Jenny").ScriptUnhide()
        #    Find("DoW_1").ScriptUnhide()
        #    Find("DoW_2").ScriptUnhide()
        #    Find("jenny_talk_timer").Enable()
        #    Find("Jenny").SetScriptedDiscipline("potence 5")
        #else:
	    G.dog1_death = 1
	    G.dog2_death = 1
	    G.jenny_death = 1

#TEMPLE2: check "dog of war" for death on map temple 2
def SabbathDogsCheck():
    if (G.dog1_death == 1): Find("DoW_1").Kill()
    if (G.dog2_death == 1): Find("DoW_2").Kill()
    if (G.jenny_death == 1):
        Find("Jenny").Kill()
        Find("jenny_talk_timer").Disable()

#TEMPLE3: check "dog of war" for death on map temple 3
def SabbathDogsCheck1():
    if (G.dog1_death == 1): Find("DoW_1").Kill()
    if (G.dog2_death == 1): Find("DoW_2").Kill()
    if (G.jenny_death == 1):
        Find("Jenny").Kill()
...
#TEMPLE4: check "dog of war" for death on map temple 4
def SabbathDogsCheck2():
    if (G.dog1_death == 1): Find("DoW_1").Kill()
    if (G.dog2_death == 1): Find("DoW_2").Kill()
    if (G.jenny_death == 1):
        Find("Jenny").Kill()
...
#TEMPLE4: Jenny starts dialog with Ming
def SabbathDialog():
    if (not G.jenny_death == 1):
        Find("Jenny_start").BeginSequence()
        if (G.dog1_death == 1): Find("DoW_1").Kill()
        if (G.dog2_death == 1): Find("DoW_2").Kill()
    else:
        Find("MingXiao2").StartPlayerDialogRemote()
	Find("Jenny").Kill()
        if (G.dog1_death == 1): Find("DoW_1").Kill()
        if (G.dog2_death == 1): Find("DoW_2").Kill()

#------------------------------------------------------------------
#TEST: NPC explosive_bot explode
def BotExplode():
...
def SubZeroCasts1():
       consoleutil.console("vdmg 5")

#------------------------------------------------------------------
#TEMPLE3: New way check if all 4 pedestals are set
def TeleportCheckPedestals():
    print "temple3: checking pedestal states..."
    if (G.tpl3_teleport_final == 1):
        print "temple3: pedestals all set!"
        tele_emitter = Find( "teleport_emitter" )
        tele_emitter.TurnOn()
        tele_trigger = Find( "teleport_trigger" )
